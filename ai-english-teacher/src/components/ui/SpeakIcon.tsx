...
export default function SpeakIcon({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        "inline-flex items-center justify-center shrink-0 w-12 h-12 min-w-12 min-h-12 rounded-full",
        "bg-pink-50 text-pink-600 border border-pink-200",
        "hover:bg-pink-100 active:scale-95 transition-all",
        playing && "bg-green-50 text-green-600 border-green-200",
        className
      )}
    >
      {playing ? <Volume1 className="w-6 h-6 animate-pulse" /> : <Volume2 className="w-6 h-6" />}
    </button>
  );
}
