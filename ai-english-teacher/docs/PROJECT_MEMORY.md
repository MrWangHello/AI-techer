...
### 5.6 宠物切心情重新加载 / 回来再等 `[preview]`

| 项 | 内容 |
|----|------|
| 现象 | 第一次慢；开心/困切换空白；离开宠物页再回来又转圈 |
| 根因 | 一个 `<video>` 换 `src` 并 `key` 重挂；切 Tab 卸载 Cat3D |
| 修复 | 五路 MP4 叠层，只改透明度；只播当前路；宠物页保活在屏外，不卸载 |
| 文件 | `Cat3D.tsx`, `page.tsx`, `PetTab.tsx` |
